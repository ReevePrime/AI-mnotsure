...
JUDGE_MODEL = OllamaJudge("llama3.1:8b")
...
def build_test_cases(dataset: list[dict]) -> list[LLMTestCase]:
    ...
    For each pair, w call chatbot.ask() to get the pipeline's actual answer.
    ...
    for i, item in enumerate(dataset):
        ...
        case = LLMTestCase(
            input=question,                           # The question that was asked
            actual_output=result["answer"],           # The answer generated by the pipeline
            expected_output=expected_answer,          # needed for ContextualRecall
            retrieval_context=result["context_used"], # the chunks passed to the LLM
            context=result["context_used"],           # needed for HallucinationMetric
        )
